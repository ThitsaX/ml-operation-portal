...
public class ModifyRoleActionGrantListController {

    private static final Logger LOG = LoggerFactory.getLogger(
        ModifyRoleActionGrantListController.class);

    private final ModifyRoleGrantList modifyRoleGrantList;

    private final ObjectMapper objectMapper;

    @PostMapping("/secured/modifyRoleGrantList")
    public ResponseEntity<Response> execute(@Valid @RequestBody Request request)
        throws DomainException, JsonProcessingException {

        LOG.info(
            "Modify Role Grant List Request : [{}]", this.objectMapper.writeValueAsString(request));
        ...
        var output = this.modifyRoleGrantList.execute(
            new ModifyRoleGrantList.Input(
        ...
        LOG.info(
            "Modify Role Grant List Response : [{}]",
    ...
    public record Request(@NotBlank @JsonProperty("roleId") String roleId,
