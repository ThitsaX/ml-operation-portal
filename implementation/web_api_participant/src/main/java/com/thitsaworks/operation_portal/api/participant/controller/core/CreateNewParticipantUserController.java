    ...
    public ResponseEntity<Response> execute(@Valid @RequestBody Request request)
        ...
        CreateNewParticipantUser.Output output = this.createNewParticipantUser.execute(
                new CreateNewParticipantUser.Input(request.name, new Email(request.email), request.password,
                                                   ...
                                                   request.activeStatus.equalsIgnoreCase("ACTIVE") ? PrincipalStatus.ACTIVE : PrincipalStatus.INACTIVE));

        var response = new Response(output.created());

        LOG.info("Create new participant user response : {}", this.objectMapper.writeValueAsString(response));
