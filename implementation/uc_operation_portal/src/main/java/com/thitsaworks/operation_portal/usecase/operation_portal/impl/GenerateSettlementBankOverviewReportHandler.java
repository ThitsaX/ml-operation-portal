...
public class GenerateSettlementBankOverviewReportHandler
    ...
    private static final Logger LOG = LoggerFactory.getLogger(
        GenerateSettlementBankOverviewReportHandler.class);
    ...
    protected Output onExecute(Input input) throws DomainException {
        ...
        if (getUser == null) {
            throw new ParticipantException(
                ParticipantErrors.USER_NOT_FOUND.format(input.userId().toString()));
        }
        ...
        String normalizedCurrency =
            input.currencyId() == null ? "ALL" : input.currencyId().toUpperCase(Locale.ROOT);
        ...
        ReportDownloadRequestManager.CreateOrReuseResult result = this.reportDownloadRequestManager.createPendingOrReuse(
            ReportType.SETTLEMENT_BANK_OVERVIEW, normalizedFileType, params);
