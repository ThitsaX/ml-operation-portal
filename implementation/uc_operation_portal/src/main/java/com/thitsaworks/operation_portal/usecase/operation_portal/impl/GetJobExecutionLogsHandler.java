...
public class GetJobExecutionLogsHandler
     ...
     */
    public GetJobExecutionLogsHandler(
    ...
    protected Output onExecute(Input input) throws DomainException {
        LOG.info("Fetching job execution logs with filters: jobName={}, status={}, startDate={}, endDate={}",
            input.jobName().orElse("<none>"),
            input.status().orElse("<none>"),
            input.startDate().orElse("<none>"),
            input.endDate().orElse("<none>")
        );

        // Create sort object based on input
        Sort sort = Sort.by(
            input.sortDirection().orElse(Sort.Direction.DESC),
            input.sortBy().orElse(GetJobExecutionLogs.DEFAULT_SORT_FIELD)
        );
        ...
        else if (input.status().isPresent()) {
        ...
        // Case 3: Filter by date range if specified
            ...
            try {
                ...
                throw new IllegalArgumentException("Invalid date format. Expected ISO-8601 format (e.g., 2023-01-01T00:00:00)");
            }
        }
