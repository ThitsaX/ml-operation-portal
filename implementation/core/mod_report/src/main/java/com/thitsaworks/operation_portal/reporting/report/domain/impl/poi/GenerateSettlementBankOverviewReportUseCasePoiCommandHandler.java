...
@Primary
@NoLogging
public class GenerateSettlementBankOverviewReportUseCasePoiCommandHandler
        implements GenerateSettlementBankOverviewReportUseCaseCommand {

    private static final Logger LOG = LoggerFactory.getLogger(
            GenerateSettlementBankOverviewReportUseCasePoiCommandHandler.class);
    ...
    public GenerateSettlementBankOverviewReportUseCasePoiCommandHandler(
    ...
    private byte[] exportSingleChunkXlsx(Input input) throws IOException, ReportException {
        ...
        Path tempFile = Files.createTempFile("settlement-bank-overview-usecase-", ".xlsx");
        ...
        try (SXSSFWorkbook workbook = new SXSSFWorkbook(DEFAULT_ROW_WINDOW);
            ...
            rowIndex = this.writeMeta(sheet, rowIndex, "Currency", this.normalizeCurrency(input.currencyId()),
                                      metaLabelStyle, metaValueStyle);
            rowIndex = this.writeMeta(sheet, rowIndex, "TimeZoneOffSet", this.displayOffset(input.timezoneOffset()),
                                      metaLabelStyle, metaValueStyle);
            rowIndex++;
            ...
            this.streamRows(input, row -> {
                ...
                this.writeDataRow(sheet.createRow(cursor[0]++), row, textCellStyle, amountCellStyle, rightTextCellStyle);
            });
            this.flushSheet(sheet);

            if (cursor[0] == rowIndex) {
    ...
    private byte[] exportPdf(Input input) throws IOException, ReportException, DocumentException {
        ...
        Path tempFile = Files.createTempFile("settlement-bank-overview-usecase-", ".pdf");
        ...
        try (OutputStream outputStream = Files.newOutputStream(tempFile)) {
            ...
            this.addPdfMetaRow(metaTable, "Settlement Created Date", metadataCapture.createdDate(), labelFont,
                               normalFont);
            this.addPdfMetaRow(metaTable, "Currency", this.normalizeCurrency(input.currencyId()), labelFont, normalFont);
            this.addPdfMetaRow(metaTable, "TimeZoneOffSet", this.displayOffset(input.timezoneOffset()), labelFont,
                               normalFont);
